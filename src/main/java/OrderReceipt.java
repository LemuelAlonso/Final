...
public class OrderReceipt extends javax.swing.JFrame {
    ...
    public OrderReceipt() {
     initComponents();
        setUndecorated(true);
        setBuyerNameFromFile("C:\\Users\\lemuel\\OneDrive\\Documents\\NetBeansProjects\\mavenproject5\\src\\main\\java\\new 4.txt");
        setCurrentDateTime();
        setDeliveryAddressFromFile("C:\\Users\\lemuel\\OneDrive\\Documents\\NetBeansProjects\\mavenproject5\\src\\main\\java\\orderReceipt5.txt"); // Read delivery address
        setStoreName(); // Set the store name
        setStoreAddress(); // Set the store address
        loadTableData("C:\\Users\\lemuel\\OneDrive\\Documents\\NetBeansProjects\\mavenproject5\\src\\main\\java\\new 3.txt"); 
    Savebtn.addActionListener(evt -> {
           saveReceiptAsImage();// Save the receipt to a file
            navigateToCartPage(); // Navigate back to CartPage
        });
 ...
 private void setCurrentDateTime() {
  ...
  private void setBuyerNameFromFile(String filePath) {
        String buyerName = readBuyerNameFromFile(filePath);
        if (buyerName != null && !buyerName.isEmpty()) {
            buyerNameField.setText(buyerName);
        } else {
            buyerNameField.setText("Unknown Buyer"); // Fallback if no name is found
        }
        buyerNameField.setEditable(false); // Make the field non-editable
    }
 private void setEmailFromFile(String filePath, String buyerName) {
        ...
        if (email != null && !email.isEmpty()) {
            ...
            emailField.setText("Email not found"); // Fallback if no email is found
        }
        emailField.setEditable(false); // Make the field non-editable
    }
    ...
    private String readEmailFromFile(String filePath, String buyerName) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 4 && parts[0].trim().equalsIgnoreCase(buyerName)) {
                    email = parts[3].trim(); // Assuming the email is the fourth part
...
// Method to read the buyer's name from the specified file
...
private void clearFile(String filePath) {
    ...
    private void initComponents() {
        ...
        getContentPane().add(deliveryField, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 200, 240, 30));
